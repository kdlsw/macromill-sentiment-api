# Macromill Sentiment Analysis API - Docker Deployment

A production-ready Docker container for the Macromill Sentiment Analysis API. This image includes pre-trained models for sentiment classification and supports both inference and training workflows.
...
- **Training Support**: Can retrain models inside the container
...
# Run the container
docker run -d -p 8000:8000 --name macromill-sentiment-api-cpu macromill/sentiment-api:latest
```
...
docker run -d \
  -p 8000:8000 \
  --name macromill-sentiment-api-cpu \
  macromill/sentiment-api:latest
```
...
```bash
docker run -d -p 9000:8000 macromill/sentiment-api:latest
```
...
```bash
# Train TF-IDF model
docker exec macromill-sentiment-api-cpu python -m macromill_sentiment.cli train --model tfidf_lr

# Train RoBERTa model (requires GPU for best performance)
docker exec macromill-sentiment-api-cpu python -m macromill_sentiment.cli train --model roberta
```

### Available Training Options

```bash
docker exec macromill-sentiment-api-cpu python -m macromill_sentiment.cli --help
```
...
docker inspect --format='{{.State.Health.Status}}' macromill-sentiment-api-cpu
...
# Or using docker
docker stop macromill-sentiment-api-cpu
docker rm macromill-sentiment-api-cpu
```
...
Check logs:
```bash
docker logs macromill-sentiment-api-cpu
```
...
```bash
docker run -d -m=2g -p 8000:8000 macromill/sentiment-api:latest
```
...
docker stop macromill-sentiment-api-cpu && docker rm macromill-sentiment-api-cpu
